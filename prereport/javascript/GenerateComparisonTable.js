 ...
 */
function createComaprisonTableOneProj(data, proj) {
    var allArchiveData = data.response;
    var allProjects = allArchiveData.allProjects;
    if (allArchiveData[proj] != undefined) {
        ...
        if (allArchiveData[proj].length > 0) {
          ...
          //  content += "<td> History </td>";
            content += "</tr><tr>";
            for (var j = 0; j < allArchiveData[proj].length; j++) {
                if (currRow > 1) {
                    currRow = 0;
                }
                var oneResrc = allArchiveData[proj][j];
                if (oneResrc != undefined && oneResrc['resource_name'] != undefined) {
                  //  content += "<tr class=" + altColor[currRow] + "><td>" + proj + "</td>";
                    content += "<tr class=" + altColor[currRow] + ">";
            ...
            content += "<tr><td>No changes Found</td></tr>";
        }
        content += "</table>";
        // alert(content);
...
function ChangeRegion() {
    ...
    location.href = 'getArchiveData.php?project=' + regionSelected;
}
