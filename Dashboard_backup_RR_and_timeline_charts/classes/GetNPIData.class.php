...
class GetNPIData {
    ...
    function createNPIData() { 
        ...
        $response = array();
        $responseData = array();
        $allMilestoneArr = array();
        $dataArr = $parse->parse($GLOBALS['NPIFetchColArr']); 
//             $allRRHtmlArr = $this->getAllRRHtml($dataArr['npi_id_arr']);
//             $dataArr['npi_id_arr'] = NULL;
//            unset($dataArr['npi_id_arr']); 
        //echo print_r($allRRHtmlArr);
        ...
        foreach ($dataArr as $oneRow) {
            ...
            //DO RR
            //echo print_r(array_search($oneRow["NPI ID"],$allRRHtmlArr));
//            if(array_key_exists($retOneRow["NPI_ID"],$allRRHtmlArr)){ 
//                $retOneRow["rrHtml"]=   $allRRHtmlArr[$retOneRow["NPI_ID"]];
//                 $phase = $retOneRow["Current_Phase"];
//               // $this->RRCalculateColor($phase, $retOneRow["rrHtml"] );
//            }
            ...
            $financialsArray["Total_Revenue"] = $this->calculateBiggerBetter($oneRow["Total Revenue (\$M) Act"], $oneRow["Total Revenue (\$M) Fst"], $oneRow["Total Revenue (\$M) Tgt"]);
            $financialsArray["Total_Revenue_Tgt"] = $oneRow["Total Revenue (\$M) Tgt"];
            $financialsArray["Qual"] = $this->getColorDate($oneRow["Qual Compl Act"], $oneRow["Qual Compl Fst"], $oneRow["Qual Compl Tgt"]);
            $financialsArray["Qual_Tgt"] = $this->getDate($oneRow["Qual Compl Act"], $oneRow["Qual Compl Fst"], $oneRow["Qual Compl Tgt"], 1);
...
//              echo "Project tgt".$retOneRow['Project']." ".$this->calculateGrossMargin($oneRow["Total Revenue (\$M) Tgt"] , $oneRow["Gross Margin (%) Tgt"])."<br>";
            ...
            $allMilestoneArr[$retOneRow["Project"]] = $milestonesArray;;
            $retOneRow["milestonesCellColor"] = $this->_milestonesColor;
            $responseData[] = $retOneRow;
        }
        $response["allDataRows"] = $responseData;
        
        $response["bubbleChartDataArr"] = $this->createBubbleChart($allMilestoneArr);
        //echo print_r($allMilestoneArr);
    ...
    function createBubbleChart($allMilestonesArr){
        $requiredMilestones = $GLOBALS['requiredMilestones'] ;
        //echo print_r($requiredMilestones);
        $chartDataTable = array();
        foreach($allMilestonesArr as $projName => $oneProjectMilestoneArr){
            
            foreach($requiredMilestones as $oneMilestoneKey){
//                echo $projName."<br>";
//                 echo print_r($oneProjectMilestoneArr);
//                echo "<br>";
                $oneDataTableArr = array();
                $oneDataTableArr[]= $oneMilestoneKey;
                $oneDataTableArr[]=$projName;
                $dateVal=$oneProjectMilestoneArr[$oneMilestoneKey]["cellValue"];
                if( empty($dateVal) || !isset($dateVal) || $dateVal == "" || $dateVal == NULL ){
                    continue;
                }
                 
                $oneDataTableArr[]=$dateVal;
                $oneDataTableArr[]=$dateVal;
                 $chartDataTable[]= $oneDataTableArr;
            }
           
            
        }
        //echo print_r($chartDataTable);
    ...
    function getDate($actual, $forecast, $target, $bString = 0) {
    ...
    function getRRHtml($npi_id) {
     ...
     function getAllRRHtml($npi_id_Arr) {
        $retArr = array();
        $rrHtml="";
        if (!isset($npi_id_Arr) || empty($npi_id_Arr)) { 
            return $retArr;
        }
        $npi_id_valueStr =  implode(" , ", $npi_id_Arr);
        $qmsComponentIdSqlResult = $this->_seaweb_buffer_db_mysqli->query("SELECT npi_id, rrwrap FROM ip_gear WHERE npi_id in (" . $npi_id_valueStr.") ");
        if (mysqli_num_rows($qmsComponentIdSqlResult) > 0) {
            while ($oneRow = mysqli_fetch_array($qmsComponentIdSqlResult)) {
                    if(isset($oneRow["rrwrap"]) && !empty($oneRow["rrwrap"]) && $oneRow["rrwrap"] != ""){
                            $rrHtml = $oneRow['rrwrap']; 
                    } else {
                            $rrHtml = "No QMS found for this Project";
                    }
                    if (!isset($rrHtml) || empty($rrHtml) || $rrHtml == NULL || $rrHtml == ""){
                        $rrHtml = "No right Review found for the project.";
                    }
                    $retArr[$oneRow["npi_id"]]=$rrHtml;
            }
        }
        return $retArr;
    }
