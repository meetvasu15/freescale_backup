...
class PurchaseService {
    ...
    function saveUpdatePurchase($purchaseObj) {
        ...
        if (isset($_GET["project"]) && !empty($_GET["project"])) {
            $project = $purchaseObj["project"];
             $lengthYet = $lengthYet + 1;
        } 
         $recurring = "";
        if (isset($_GET["recurring"]) && !empty($_GET["recurring"])) {
            $recurring = $purchaseObj["recurring"];
             $lengthYet = $lengthYet + 1;
        }
         $priority = "";
        if (isset($_GET["priority"]) && !empty($_GET["priority"])) {
            $priority = $purchaseObj["priority"];
             $lengthYet = $lengthYet + 1;
        }
        $saveUpdateSqlQuery = "";
        if (FALSE === array_key_exists("system_id", $purchaseObj)) {
            ...
            if (isset($pr_number) && !empty($pr_number) && $pr_number !== ""){
                $saveUpdateSqlQuery .= " pr_number= " . $pr_number;
                 ...
                 $saveUpdateSqlQuery .= " pr_number=0 ";
            }
            if (isset($project) && !empty($project) && $project !== "")
                $saveUpdateSqlQuery .= ", project= '" . $project."'";
            if (isset($priority) && !empty($priority) && $priority !== "")
                $saveUpdateSqlQuery .= ", priority= '" . $priority."'";
            if (isset($recurring) && !empty($recurring) && $recurring !== "")
                $saveUpdateSqlQuery .= ", recurring= '" . $recurring."'";
