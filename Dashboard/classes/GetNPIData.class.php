...
class GetNPIData {
    ...
    function createNPIData() {
        ...
        $rrstatus = $this->getAllRRHtml($npi_id_Arr);
         $response["rrstatusColor"] = $rrstatus;
    ...
    function getQMSData($projectName) {
     ...
     function getIpQMSData($obj_id) {  
        $retArr = array();
        if (!isset($obj_id) || empty($obj_id)) {
            echo "SEVERE ERROR: Got no npi id!";
            return;
        } 
        $qmsHtmlSqlResult = $this->_seaweb_buffer_db_mysqli->query("SELECT extra_info FROM ip_gear WHERE pv_objid = " . $obj_id);
        if (mysqli_num_rows($qmsHtmlSqlResult) > 0) {
            while ($oneRow = mysqli_fetch_array($qmsHtmlSqlResult)) {
                $retHtml = $oneRow['extra_info'];
            }
  ...
  function getAllRRHtml($npi_id_Arr) {
        ...
        $qmsComponentIdSqlResult = $this->_seaweb_buffer_db_mysqli->query("SELECT npi_id, rr_status FROM ip_gear WHERE npi_id in (" . $npi_id_valueStr.") ");
        if (mysqli_num_rows($qmsComponentIdSqlResult) > 0) {
            while ($oneRow = mysqli_fetch_array($qmsComponentIdSqlResult)) {
                     $rrHtml="";
                    if(isset($oneRow["rr_status"]) && !empty($oneRow["rr_status"]) && $oneRow["rr_status"] != ""){
                            $rrHtml = $oneRow['rr_status'];
                //              echo $oneRow['rr_status'];
                    }
                      //$retArr[$oneRow["npi_id"]]="RED";
