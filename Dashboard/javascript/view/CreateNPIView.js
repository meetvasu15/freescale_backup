...
function  CreateNpiTableView(data) {
    ...
    var allRRStatusCode = NpiDataModel.getInstance().npiData.response.rrstatusColor;
...
//vasu changes begin 10-09 
    ...
    npiContent += "<th  class='filter-false wrap-td' style='width: 10%;' data-placeholder=''>Vertical Market</th>";
    npiContent += "<th class='filter-false wrap-td'  style='width: 8%;'   data-placeholder=''>PM</th>";
    ...
    for (var i = 0; i < allDataRows.length; i++) {
        ...
        npiContent += "<tr>";
        npiContent += "<td  class='wrap-td'  style='width: 5%;' >" + allDataRows[i].region + "</td>";
        npiContent += "<td class='wrap-td'  style='width: 17%;'><a href='http://webnpi.am.freescale.net/docs/TSPG/MCD/NPI/HTML/npi_projects.php?ProjectDisplaying,ID=" + allDataRows[i].NPI_ID + "' target='WebNPI'>" + allDataRows[i].Project + "</a>";
        ...
        npiContent += "</td>";
        npiContent += "<td  class='wrap-td'   style='width: 5%;' >" + allDataRows[i].Current_Phase + "</td>";
        npiContent += "<td  class='wrap-td' style='width: 10%;'> " + allDataRows[i].Vertical_Market_Space + "</td>";
        npiContent += "<td  class='wrap-td' style='width: 8%;'>" + allDataRows[i].Program_Manager + "</td>";
        npiContent += "<td style='width: 9%;'>" + allDataRows[i].Technology + "</td>";
        npiContent += "<td style='width: 8%;'>" + allDataRows[i].Complexity + "</td>";
        if (!isBlankString(allDataRows[i].milestones.Qual_Compl.cellValue)) {
            npiContent += "<td style='width: 6%;'>" + allDataRows[i].milestones.Qual_Compl.cellValue + "</td>";
        }
        else {
            npiContent += "<td style='width: 6%;'></td>";
        }
        npiContent += "<td style='width: 6%;'>" + allDataRows[i].cycle_time + "</td>";
        npiContent += "<td class='financialModalButton " + allDataRows[i].financialCellColor + "' onclick='GetFinancialView(" + i + ")' style='width: 6%;'> View ($)</td>";
        npiContent += "<td class='qmsModalButton'  style='width: 4%;'> <span class='label' onclick='GetQMSHtml(\"" + encodeProjectName + "\")'>View </span><br><span class='label label-info' onclick='LaunchQmsPage(\"" + allDataRows[i].NPI_ID + "\")'>QMS </span></td>";
        if(allRRStatusCode[allDataRows[i].NPI_ID] != undefined && allRRStatusCode[allDataRows[i].NPI_ID] != ""){
            npiContent += "<td class='redRRModalButton' onclick='GetRRHtml(\"" + allDataRows[i].NPI_ID + "\")' style='width: 6%;'>RR</td>";
        }else{
            npiContent += "<td class='rrModalButton' onclick='GetRRHtml(\"" + allDataRows[i].NPI_ID + "\")' style='width: 6%;'>RR</td>";
        }
        npiContent += "<td class='milestonesModalButton " + allDataRows[i].milestonesCellColor + "' onclick='GetMilestonesView(" + i + ")' style='width: 6%;'>Milestones</td>";
//         npiContent +="<td>IP</td>";
//        npiContent +="<td>IP disconnects</td>";
        npiContent += "<td  class='burndownModalButton' onclick='createBurnDownModalWindow(" + i + ")' style='width: 9%;'>Charts</td>";
        npiContent += "</tr>";
    }
    npiContent += "</table>";
   ...
   //showHideColumn();
    //  $('#allNPIContentTable').tableScroll({height:700});  
    $("#allNPIContentTable").freezeHeader();
}
...
function showHideColumn() {
    var isVisible = $('#allNPIContentTable td:nth-child(3)').is(":visible");
    if (isVisible) {
        //$('#allNPIContentTable td:nth-child(3), td:nth-child(4), td:nth-child(5), td:nth-child(6), td:nth-child(7), td:nth-child(8)').hide();
        $('#allNPIContentTable th:nth-child(3),td:nth-child(3),th:nth-child(4),td:nth-child(4),td:nth-child(5),th:nth-child(5),td:nth-child(6),th:nth-child(6),td:nth-child(7),th:nth-child(7),td:nth-child(8),th:nth-child(8)').hide();
        
        $("#npiProjectHead").html("<a href='#'>More Details</a>");
    } else {
        // $('#allNPIContentTable td:nth-child(3), td:nth-child(4), td:nth-child(5), td:nth-child(6), td:nth-child(7), td:nth-child(8)').show();
         $('#allNPIContentTable th:nth-child(3),td:nth-child(3),th:nth-child(4),td:nth-child(4),td:nth-child(5),th:nth-child(5),td:nth-child(6),th:nth-child(6),td:nth-child(7),th:nth-child(7),td:nth-child(8),th:nth-child(8)').show();
       $("#npiProjectHead").html("<a href='#'>Less Details</a>");
