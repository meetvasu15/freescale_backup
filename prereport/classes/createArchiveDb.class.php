...
require_once 'classes/SingletondevDb.class.php';
...
class createArchiveDb {
    ...
    function createArchiveDb() {
        ...
        $db = SingletonDb::getInstance();
         $archivedb = SingletondevDb::getInstance();
        //    echo "db";
        $this->_archive_data_db_mysqli = $archivedb->getSeawebArchiveDbConnection();
    ...
    function refreshArchiveDb($zippedDumpFileName) {
        ...
        $createTableSql = "CREATE TABLE seaweb_pv_archive_db.archiveDate (archive VARCHAR(100))";
//	echo $createTableSql."<br>";   
        $this->_archive_data_db_mysqli->query($createTableSql);
        $insertDate = "INSERT INTO seaweb_pv_archive_db.archiveDate VALUES ('" . $zippedDumpFileName . "')";
//	echo $insertDate."<br>";         
        $this->_archive_data_db_mysqli->query($insertDate);
    }

    function getArchiveDbDate() {
        $archiveDate = "";
        $getarchiveDateMysqlQuery = "SELECT archive FROM archiveDate";
        $allarchiveDateSqlResult = $this->_archive_data_db_mysqli->query($getarchiveDateMysqlQuery);
        if ($allarchiveDateSqlResult && mysqli_num_rows($allarchiveDateSqlResult) > 0) {
            while ($oneRow = mysqli_fetch_array($allarchiveDateSqlResult)) {
                $archiveDate = $oneRow['archive'];
            }
        } else {
    ...
    function getArchiveDataComparison($project) {
        if (isset($project) && !empty($project) && $project != "") {
            $archive = $this->getResourceAvailData($project, $this->_archive_data_db_mysqli);
            $current = $this->getResourceAvailData($project, $this->_current_pv_data_db_mysqli);
            $retArr["response"] = $this->comparePerProjAvailability($archive, $current, $project);
        }
        $retArr["allDates"] = $current["allDates"];
        $retArr["allProjects"] = $this->getProjects($this->_current_pv_data_db_mysqli);
        $retArr["archiveDate"] = $this->getArchiveDbDate();
        return $retArr;
        //echo "56<br>";
    }
    ...
    function compareAvailability($archive, $current) {
        ...
        foreach ($allProjectsArr as $oneRoleResources) {
            ...
            foreach ($allArchiveResources as $oneResource) {
                // $oneVaryingAvailabiltySrc["toDisplay"] = FALSE;
                if ($oneResource["resourceobjid"]) {
                    $oneVaryingAvailabiltySrc = array();
                    $currResource = $allCurrentResourceArr[$oneResource["resourceobjid"]];
                    $oneVaryingAvailabiltySrc["resource_name"] = $currResource["resource_name"];
                    $oneVaryingAvailabiltySrc["role"] = $currResource["prim_role_id"];
                    $oneVaryingAvailabiltySrc['history'] = $currResource['history'];
                    $oneVaryingAvailabiltySrc["changed"] = FALSE;
                    $dateChanges = array();
                    $count = 0;
                    foreach ($allDates as $oneDate) {

                        if ($currResource[$oneDate] && $oneResource[$oneDate] && $currResource[$oneDate] != $oneResource[$oneDate] //&& FALSE === array_search($currResource["resourceobjid"], $allInserted)
                        ) {
                            $oneVaryingAvailabiltySrc["changed"] = TRUE;
                            $obj = array();
                            $curr = $currResource[$oneDate];
                            $old = $oneResource[$oneDate];
                            $obj[0] = $oneDate;
                            $obj[1] = $old . "% - " . $curr;

                            //echo "Name ". $oneVaryingAvailabiltySrc["resource_name"]." vals ".  print_r($obj)."<br>"; 
                            $dateChanges[$count++] = $obj;
                    ...
                    if ($oneVaryingAvailabiltySrc["changed"]) {
                        $oneVaryingAvailabiltySrc["dateChanges"] = $dateChanges;

                        $oneProjectArr[] = $oneVaryingAvailabiltySrc;
                    }
                }
            }
...
return $retArray;
    ...
    function getResourceAvailData($project, $whichDb) {
//         return;
        // echo "0<br>";
        $retArray = array();
        $dataArray = array();
        ...
        $allDateColSqlResult = $whichDb->
                query("SELECT DISTINCT startdate 
                       ...
                       WHERE startdate >= '" . date('Y-m-01', strtotime('-10 month')) . "'
                       AND startdate <= '" . date('Y-m-01', strtotime('+10 month')) . "'");
        $count = 0;
//	echo "::::<br>SELECT DISTINCT startdate
//                       FROM resourcespread_tb
//                       WHERE startdate >= '" . date('Y-m-01', strtotime('-10 month')) . "'
//                       AND startdate <= '" . date('Y-m-01', strtotime('+10 month')) . "'<br>::::";

        //     echo print_r($allProjectsArr)."<br>";
        ...
        //get availabilty
        $getResourceAvailabilityQuery = "SELECT  assignment.resource_name, val.resourceobjid,assignment.project_name , val.history , assignment.prim_role_id ," . $concat . " 
            FROM  resourceassignment_tb assignment , resourcespread_tb val
            Where assignment.objid = val.resourceobjid 
            AND assignment.active ='true'
             AND assignment.project_name = '$project'
            GROUP BY val.resourceobjid";
	//echo "::::<br>".$getResourceAvailabilityQuery."<br>::::";
        $allResourceAvailabilitySqlResult = $whichDb->query($getResourceAvailabilityQuery);
        //      echo $getResourceAvailabilityQuery."<br><br><br>";
        while ($oneRow = mysqli_fetch_array($allResourceAvailabilitySqlResult)) {
            ...
            $oneResource['prim_role_id'] = $oneRow['prim_role_id'];
            $oneResource['history'] = $oneRow['history'];
...
//echo print_r($allProjectsArr)."<br><br><br>";
        return $allProjectsArr;
    }

    function getProjects($whichDb) {
        $getProjectsMysqlQuery = "SELECT DISTINCT id FROM project_tb WHERE region IS NOT NULL";
        //$getProjectsMysqlQuery = "SELECT id DISTINCT FROM project_tb";
        //echo "<br>project regional <br>" . $getProjectsMysqlQuery . "<br>";
        $allProjectsSqlResult = $whichDb->query($getProjectsMysqlQuery);
        $allProjectsArr = array();
...
//echo print_r($allProjectsArr)."<br><br><br>";
    ...
    function comparePerProjAvailability($archive, $current, $oneProj) {
        $retArray = array();
        $allInserted = array();
        $allArchiveRolesResourceArr = $archive["allRoleResources"];
        $allCurrentResourceArr = $current["allResourcesById"];
        ...
        $allProjectsArr = $current["allProjects"];
        //echo "current object received<br>".print_r($allCurrentRolesResourceArr)."<br>"; 
        $oneProjectResourceArr = array();
        if ($allArchiveRolesResourceArr[$oneProj]) {
            $allArchiveResources = $allArchiveRolesResourceArr[$oneProj]["allResources"];
            //  $allCurrentResources = $allCurrentRolesResourceArr[$oneProj]["allResources"];
            foreach ($allArchiveResources as $oneResource) {
                // $oneVaryingAvailabiltySrc["toDisplay"] = FALSE;
                if ($oneResource["resourceobjid"]) {
                    $oneVaryingAvailabiltySrc = array();
                    ...
                    if ($oneVaryingAvailabiltySrc["changed"]) {
                        ...
                        $oneProjectResourceArr[] = $oneVaryingAvailabiltySrc;
                    }
                }
            }
            $retArray[$oneProj] = $oneProjectResourceArr;
        }
        $retArray["allProjects"] = $allProjectsArr;
        return $retArray;
    }
